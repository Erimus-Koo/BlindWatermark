...
if __name__ == "__main__":
    ...
    src = os.path.join(here, '../pic/lena512p.png')
    wm = os.path.join(here, '../pic/wm32.png')
    out = os.path.join(here, '../out_old.png')
    out_wm = os.path.join(here, '../out_old_wm.png')

    bwm1 = watermark(4399, 2333, 32)
    bwm1.read_ori_img(src)
    bwm1.read_wm(wm)
    bwm1.embed(out)

    bwm1.extract(out, out_wm)
